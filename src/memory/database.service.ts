...
export class DatabaseService implements OnModuleInit {
  ...
  async saveDocument(data: any) {
    const result = await this.pool.query(
      `INSERT INTO documents (filename, summary, text, invoice_data, company_id)
  ...
  async saveInvoice(data: any) {
    const result = await this.pool.query(
      `INSERT INTO invoices (client_name, amount, currency, date, description, company_id)
  ...
  async saveCommand(data: any) {
    try {
      await this.pool.query(
        `INSERT INTO commands (command, response, lang, intent, company_id)
         VALUES ($1, $2, $3, $4, $5)`,
